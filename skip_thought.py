...
def make_batch(seq_data):
    ...
    for seq in seq_data:
        temp = seq['encode'].split()
        temp2 = []
        temp3 = []
        for i in temp:
            temp2.append(word_to_id[i])
            if i == "<p>":
                temp3.append(0)
            else:
                temp3.append(1)

        encode_batch.append(temp2)
        mask.append(temp3)

        temp = seq['decode_pre'].split()
        temp2 = []
        temp3 = []
        for i in temp:
            temp2.append(word_to_id[i])
    ...
    return encode_batch, pre_batch, pos_batch, mask, pre_mask, pos_mask
...
def _initialize_gru_cell(self, num_units):
    return gru_cell.LayerNormGRUCell(
        ...
        b_initializer=tf.constant_initizlier(0.0))
...
with tf.variable_scope("encoder") as scope:
    ...
    _, state = tf.nn.dynamic_rnn(
        ...
        dtype=tf.float32,
        scope=scope)
    thought_vectors = tf.identify(state, name="thought_vectors")
...
optimizer = tf.train.AdamOptimizeer(learning_rate)
